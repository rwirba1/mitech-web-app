        ...
        stage('Terraform Actions') {
            steps {
                script {
                        ...
                        env.EC2_PUBLIC_IP = sh(script: 'terraform output instance_public_ip', returnStdout: true).trim().replaceAll('"', '')
                 }
           }
       }
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'ansible-ssh-agent', keyFileVariable: 'SSH_KEY')]) {
                    sshagent(credentials: ['ansible-ssh-agent']) {
                        ...
                        ansible-playbook -i "${EC2_PUBLIC_IP}," /home/ubuntu/workspace/devops-demo/install.yml --private-key=$SSH_KEY -u ubuntu -e target="${EC2_PUBLIC_IP}"
                        '''
                    }
                }
            }
